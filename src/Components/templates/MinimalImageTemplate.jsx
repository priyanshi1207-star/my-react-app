...
const MinimalImageTemplate = ({ data, accentColor }) => {
    // Helper to format date strings from YYYY-MM to "MMM YYYY"
            ...
            <div className="grid grid-cols-12 gap-12">
                ...
                <aside className="col-span-4 border-r border-zinc-100 pr-10">
                    ...
                    {data.education && data.education.length > 0 && (
                        <section className="mb-10">
                            <h2 className="text-xs font-bold tracking-[0.2em] text-zinc-400 uppercase mb-5">Education</h2>
                        ...
                        <section className="mb-10">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {data.skills.map((skill, index) => (
                                    <span key={index} className="px-2 py-1 bg-zinc-50 border border-zinc-200 text-[11px] rounded text-zinc-600">
                                        {skill}
                                    </span>
                                ))}
                            </div>
                ...
                <main className="col-span-8">
                    ...
                    {data.professional_summary && (
                        <section className="mb-12">
                            <h2 className="text-xs font-bold tracking-[0.2em] text-zinc-400 uppercase mb-5">Profile</h2>
                            <p className="text-sm text-zinc-600 leading-relaxed italic">{data.professional_summary}</p>
                        </section>
                    )}
                            ...
                            <div className="space-y-10">
                                {data.work_experience.map((exp, index) => (
                                    <div key={index}>
                                        ...
                                        <p className="text-sm text-zinc-600 leading-relaxed whitespace-pre-line">{exp.description}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
                        ...
                        <section className="mb-12">
                            ...
                            <div className="space-y-6">
                                {data.projects.map((proj, index) => (
                                    <div key={index} className="p-4 bg-zinc-50 rounded border-l-2" style={{ borderLeftColor: accentColor }}>
