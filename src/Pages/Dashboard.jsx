    ...
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        {showUploadResume && (
          ...
          bg-black/70 backdrop-blur bg-opacity-50
            z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md
            rounded-lg w-full max-w-sm p-6'>
                ...
                hover:text-green-700 cursor-pointer transition-colors'>
                    {resume ? (
                      ...
                      <><UploadCloud className='size-14 stroke-1' />
                        <p>Click to upload your resume (PDF, DOCX)</p>
                      </>
                    )}
                  </div>
                </label>
                <input type="file" id='resume-upload' accept='.pdf' hidden
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={() => setShowEditResume('')} className='fixed inset-0 
          bg-black/70 backdrop-blur bg-opacity-50
            z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md
            rounded-lg w-full max-w-sm p-6'>
              ...
              <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700
              transition-colors'>Edit Resume</button>
              <XIcon className='absolute top-4 right-4 text-slate-400
              hover:text-slate-600 cursor-pointer transition-colors'
                onClick={() => { setShowEditResumeId(''); setTitle('') }} />
            </div>
          </form>
        )}
